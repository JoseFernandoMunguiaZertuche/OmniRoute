...
// Typed accessor for sanitizer results (returns `unknown`): keeps assertions
// type-safe without untyped casts (zero-warning lint policy).
type EffortResult = {
  model?: string;
...
test("sanitizeReasoningEffortForProvider: xiaomi-mimo preserves xhigh by default", () => {
  ...
  assert.equal((result as EffortResult).reasoning_effort, "xhigh");
  assert.equal((result as EffortResult).model, "mimo-v2.5-pro", "other fields preserved");
...
test("sanitizeReasoningEffortForProvider: explicit xhigh opt-out downgrades to high", () => {
  ...
  assert.equal((result as EffortResult).reasoning_effort, "high");
  assert.ok(
...
test("sanitizeReasoningEffortForProvider: Ollama Cloud preserves max", () => {
  ...
  assert.equal((result as EffortResult).reasoning_effort, "max");
  assert.equal(log.messages.length, 0);
});
...
test("sanitizeReasoningEffortForProvider: Ollama Cloud preserves nested max", () => {
  ...
  assert.equal(result, body, "Ollama Cloud accepts max literally");
  assert.equal((result as EffortResult).reasoning.effort, "max");
  assert.equal((result as EffortResult).reasoning.summary, "auto");
});
...
test("sanitizeReasoningEffortForProvider: nested OpenAI reasoning max normalizes to xhigh", () => {
  ...
  assert.equal((result as EffortResult).reasoning.effort, "xhigh");
  assert.equal(
    (result as EffortResult).reasoning.summary,
    "auto",
    "other reasoning fields preserved"
  );
  assert.equal((result as EffortResult).reasoning_effort, undefined);
});
...
test("sanitizeReasoningEffortForProvider: claude preserves max for Opus/Sonnet and downgrades Haiku", () => {
  ...
  assert.equal((sonnetResult as EffortResult).reasoning_effort, "max");
  ...
  assert.equal((opusResult as EffortResult).reasoning.effort, "max");
  ...
  assert.equal((haikuResult as EffortResult).reasoning_effort, "high");
});
...
test("sanitizeReasoningEffortForProvider: explicit xhigh opt-out preserves Responses shape", () => {
  ...
  assert.equal((result as EffortResult).reasoning.effort, "high");
  assert.equal((result as EffortResult).reasoning_effort, undefined);
});
...
test("sanitizeReasoningEffortForProvider: mistral/devstral strips reasoning_effort entirely", () => {
  ...
  assert.equal(
    (result as EffortResult).reasoning_effort,
    undefined,
...
test("sanitizeReasoningEffortForProvider: rejecting providers strip max before normalization", () => {
  ...
  assert.equal((mistralResult as EffortResult).reasoning_effort, undefined);
  ...
  assert.equal((githubResult as EffortResult).reasoning_effort, "high");
  ...
  assert.equal((githubHaikuResult as EffortResult).reasoning_effort, undefined);
});
...
test("sanitizeReasoningEffortForProvider: mistral/devstral strips reasoning object when only effort present", () => {
  ...
  assert.equal(
    (result as EffortResult).reasoning,
    undefined,
...
test("sanitizeReasoningEffortForProvider: mistral/devstral preserves reasoning when other fields remain", () => {
  ...
  const result = sanitizeReasoningEffortForProvider(body, "mistral", "devstral-2512", null);
  assert.deepEqual((result as EffortResult).reasoning, { summary: "auto" });
});
...
test("sanitizeReasoningEffortForProvider: native deepseek maps xhigh → max", () => {
  ...
  assert.notEqual(result, body, "must return a new object when mutating");
  assert.equal((result as EffortResult).reasoning_effort, "max");
  assert.equal((result as EffortResult).model, "deepseek-v4-pro", "other fields preserved");
  assert.ok(
...
// ── opencode-zen DeepSeek literal max (verified 2026-07-31) ──────────────────
// opencode-zen's DeepSeek free tier accepts {high, max} literally
// (reasoning_effort=max returns 200 on deepseek-v4-flash-free). Mirrors the
// opencode-go contract from #4647.

test("sanitizeReasoningEffortForProvider: opencode-zen DeepSeek preserves max", () => {
  const body = {
  ...
  assert.equal(result, body, "opencode-zen DeepSeek max must pass through unchanged");
  assert.equal((result as EffortResult).reasoning_effort, "max");
});

// ── NVIDIA GLM-5.2 literal max (verified 2026-07-31) ─────────────────────────
// integrate.api.nvidia.com/v1/chat/completions accepts reasoning_effort=max on
// z-ai/glm-5.2 (HTTP 200). Z.ai's enum for GLM-5.2 lists max as the
// default/recommended deep-reasoning tier (xhigh is mapped to max).

test("sanitizeReasoningEffortForProvider: NVIDIA GLM-5.2 preserves max", () => {
  const body = {
    model: "z-ai/glm-5.2",
    reasoning_effort: "max",
    messages: [{ role: "user", content: "hi" }],
  };
  const result = sanitizeReasoningEffortForProvider(body, "nvidia", "z-ai/glm-5.2", null);
  assert.equal(result, body, "NVIDIA GLM-5.2 max must pass through unchanged");
  assert.equal((result as EffortResult).reasoning_effort, "max");
});

test("sanitizeReasoningEffortForProvider: NVIDIA GLM-5.2 preserves nested max", () => {
  const body = {
    model: "z-ai/glm-5.2",
    reasoning: { effort: "max", summary: "auto" },
    input: [],
  ...
  const result = sanitizeReasoningEffortForProvider(body, "nvidia", "z-ai/glm-5.2", null);
  assert.equal(result, body, "NVIDIA GLM-5.2 nested max must pass through unchanged");
  assert.equal((result as EffortResult).reasoning.effort, "max");
  assert.equal((result as EffortResult).reasoning.summary, "auto");
});

test("sanitizeReasoningEffortForProvider: NVIDIA non-GLM-5.2 models still normalize max → xhigh", () => {
  // The opt-in must be scoped to z-ai/glm-5.2 only — other NVIDIA-hosted
  // models (e.g. minimax/mistral/gemma) follow the default xhigh policy.
  const body = {
    model: "minimaxai/minimax-m2.7",
    reasoning_effort: "max",
    messages: [],
  };
  const result = sanitizeReasoningEffortForProvider(body, "nvidia", "minimaxai/minimax-m2.7", null);
  assert.notEqual(result, body);
  assert.equal((result as EffortResult).reasoning_effort, "xhigh");
});
